...
export default defineConfig({
  ...
  preview: {
    host: '0.0.0.0',
    port: parseInt(process.env.PORT || '8443') + 1,
  },
  // Pre-bundle heavy dependencies to avoid on-the-fly optimization
  optimizeDeps: {
    include: [
      'react',
      'react-dom',
  ...
  // Split heavy libraries into separate chunks for parallel loading
  build: {
    rollupOptions: {
      output: {
